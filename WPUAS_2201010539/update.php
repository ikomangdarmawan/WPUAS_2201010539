...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  //update data data ke database
  $query = "UPDATE tb_pendaftaran SET 
      seka='$seka', 
      kriteria='$kriteria'
    WHERE no_undi='$no_undi'";
...
<!-- konten -->
<div class="container">
<h5 class="text-center mt-5">TAMBAH data</h5>
<form class="shadow rounded py-4 px-3 my-4" action="update.php?id=<?= $data['no_undi']; ?>" method="POST">
  <div class="mb-3">
    <label for="no_undi" class="form-label">no_undi</label>
    <input required readonly type="text" class="form-control" id="no_undi" name="no_undi" value="<?= $data['no_undi'] ?>">
  </div>
  <div class="mb-3">
    <label for="seka" class="form-label">seka</label>
    <input required type="text" class="form-control" id="seka" name="seka" value="<?= $data['seka'] ?>">
  </div>
  <div class="mb-3">
    <label for="kriteria" class="form-label">kriteria</label>
    <select class="form-select" id="kriteria" name="kriteria">
      <option selected disabled>Choose...</option>
      <option value="Tari Pendet" <?php if ($data['kriteria'] == 'Tari Pendet') { echo 'selected'; } ?>>Tari Pendet</option>
      <option value="Tari Panji Semirang" <?php if ($data['kriteria'] == 'Tari Panji Semirang') { echo 'selected'; } ?>>Tari Panji Semirang</option>
      <option value="Tari Legong" <?php if ($data['kriteria'] == 'Tari Legong') { echo 'selected'; } ?>>Tari Legong</option>
    </select>
  </div>
